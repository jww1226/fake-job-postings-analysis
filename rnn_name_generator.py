...
def seqs_to_ids(seqs, char_to_id, max_len=20):
    ...
    for seq in seqs:
        ...
        if len(seq) == 0:
            continue
        for s in seq[:max_len]:
            rseq.append(char_to_id[s])
        rseq += [0] * (max_len - len(rseq))
        all_seqs.append(rseq)
...
def train_model(model, Xtrain, Ytrain, Xval, Yval, id_to_char, max_epoch):
    ...
    for e in range(max_epoch):
        print("Epoch %d, Val Loss: %f" % (e, calc_val_loss(model, Xval, Yval)))

        # shuffle the training data
        with torch.no_grad():
            order = torch.randperm(Xtrain.shape[0])
            Xtrain = Xtrain[order]
            Ytrain = Ytrain[order]
        ...
        for n in range(num_batches):
            ...
            # forward pass of the model using training data
            s = n*batch_size
            e = (n+1)*batch_size
            mout, _ = model(Xtrain[s:e])

            # calculate the loss
            loss = lossfn(mout.permute(0, 2, 1), Ytrain[s:e])

            # do gradient step
            loss.backward()
            optim.step()

        # print a few examples after the batch is generated
        # Note: students are not required to do this
        for i in range(3):
            print(gen_string(model, id_to_char))
        print("----")
...
def gen_string(model, id_to_char, max_len=20, sample=True):
    ...
    for i in range(max_len):
        ...
        # get the output of the model and take the softmax
        # store the last hidden output for input at the next step
        out, h = model(x,h)
        prob = nn.functional.softmax(out, dim=2)

        # either sample the best token or randomly choose it
        npprob = prob.detach().cpu().numpy()
        if sample:
            chosen = np.random.choice(npprob.shape[2], p=npprob[0, 0, :])
        else:
            chosen = np.argmax(npprob[0, 0, :])

        # if we generate the end of sequence token then we are done
        if chosen == 2:
            break

        # set the new input token as the chosen one
        x[0, 0] = chosen

        # save the chosen token to the output string
        out_str = out_str + id_to_char[chosen]
